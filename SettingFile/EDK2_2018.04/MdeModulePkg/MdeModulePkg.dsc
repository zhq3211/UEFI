...
[Components]
  ...
  #####################################################
  #													   #
  # GitHub - zhq module - build （Shell U盘下运行）				   #
  #													   #
  # > build -p MdeModulePkg\MdeModulePkg.dsc -a X64		   #
  # 生成*.efi路径：Build\MdeModule\DEBUG_VS2013x86\X64\*.efi	   # 
  #													   #
  #####################################################
  UEFI\SPD\MySPD.inf
  UEFI\SPD\MyCommonSPD.inf
  UEFI\SIO\Voltage\VoltageSmaple.inf
  UEFI\SIO\Voltage\Voltage.inf
  UEFI\SIO\Temperature\Temperature.inf
  UEFI\SIO\Fan\FanTachometer.inf
  UEFI\SIO\Fan\FanControl.inf
  UEFI\SIO\GPIO\LED_Yellow.inf
  UEFI\SIO\GPIO\LED_White.inf
